...
function summarizeToolArguments(toolName, args) {
    ...
    for (const key of preferred) {
        ...
        if (typeof value === 'string' && value.trim() !== '')
            return oneLine(value, NOTICE_COMMAND_MAX_CHARS);
    }
    try {
...
function findToolCallArguments(session, callId, toolName) {
    ...
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        ...
        for (const block of message.content ?? []) {
            if (block.type !== 'tool-call')
                continue;
            if (callId !== undefined && block.id !== callId)
                continue;
...
function collectRecentUserText(session, max) {
    const messages = session.deriveMessages?.() ?? [];
    const lines = [];
...
function buildReviewInput(req, config) {
...
export function parseLlmDecision(text) {
    ...
    try {
        ...
        const reason = typeof parsed.reason === 'string' && parsed.reason.trim() !== ''
            ...
            : undefined;
        if (action === 'allow' || action === 'allowed-once')
...
*/
function injectApprovalNotice(req, notice) {
    try {
...
async function runLlmReview(ctx, req, input, config) {
    if (!config.useLlm)
        return null;
...
function permissionIconClientPath() {
...
function permissionIconEntry(mapFormat) {
...
export function patchPermissionIconSource(source) {
    if (source.includes(ICON_MARK_OBJECT) || source.includes(ICON_MARK_MAP))
        return source;
    const anchorAt = source.indexOf(ICON_ANCHOR);
    if (anchorAt < 0)
        throw new Error('icon anchor not found (DSH bundle layout changed?)');
    ...
    if (closeAt < 0)
        throw new Error('no glyph container close found after anchor');
    if (mapFormat) {
...
export function apply(ctx, config) {
    ...
    ctx.on('approval/request', async (req, next) => {
        try {
            ...
            const noticeBase = {
                authorization: (input.hasExplicitUserConfirmation ? 'user-confirmed' : 'unknown'),
